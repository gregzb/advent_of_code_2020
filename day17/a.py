...
class Cube:
    def __init__(self, cube):
        ...
        for i in range(len(cube)):
            for j in range(len(cube[0])):
                self.cube[(j, i, 0)] = cube[i][j]
        
        self.calc_min_max()

    def calc_min_max(self):
        self.maxes = [None, None, None]
        self.mins = [None, None, None]
    ...
    def count_neighboring(self, x, y, z):
        dirs = [-1, 0, 1]
        tot = 0
        for xo, yo, zo in itertools.product(dirs, repeat=3):
            if xo==0 and yo==0 and zo==0:
                continue
            tot += int(self.cube[(x+xo,y+yo,z+zo)])
        return tot
        ...
        for x in range(self.mins[0], self.maxes[0]+1):
            for y in range(self.mins[1], self.maxes[1]+1):
                for z in range(self.mins[2], self.maxes[2]+1):
                    neighbors = self.count_neighboring(x, y, z)
                    if self.cube[(x,y,z)]:
                        if not (neighbors == 2 or neighbors == 3):
                            copy.cube[(x,y,z)] = False
                    else:
                        if neighbors == 3:
                            copy.cube[(x,y,z)] = True
